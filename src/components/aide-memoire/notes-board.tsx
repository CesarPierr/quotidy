...
export function NotesBoard({
  ...
  async function handleQuickAdd(event: FormEvent<HTMLFormElement>) {
    ...
    setIsAdding(true);
    try {
      const response = await postForm(`/api/households/${householdId}/notes`, { body: trimmed });
  ...
  return (
    <div className="space-y-3">
      {/* Quick capture */}
      <form className="flex items-center gap-2" onSubmit={handleQuickAdd}>
      ...
      {active.length === 0 ? (
        <p className="rounded-2xl border border-dashed border-line px-4 py-6 text-center text-sm text-ink-500">
          Rien à retenir pour le moment. Notez vos rappels du foyer ici.
        </p>
      ) : (
        <ul className="space-y-2" aria-label="Notes à traiter" aria-live="polite">
          {visibleActive.map((note) => (
            <li
              className="soft-panel flex items-start gap-2.5 p-2.5"
              key={note.id}
